...
import { PerformanceMeasureMemoryInformation, PerformanceMeasureMemoryBreakdown } from "@perf-tools/keeper/types/performance";
import { UserAgentSpecificTypes } from "@perf-tools/keeper/types/userAgent";
import { createTimingsGroup } from "../perfKeeperNavigation/perf-navigation-utils";
import { PerfKeeper } from "../perfMetrica/perf.types";
import { createInterval } from "./perf-memory-util";
...
export function measureMemory(keeper: PerfKeeper, options: MeasureMemoryOptions = {}) {
	...
	domReady(() => {
		createInterval('ready', options.intervals, (group) => (performance as any).measureMemory!().then((info: any) => {
			...
			for (let item of breakdown) {
				...
				item.userAgentSpecificTypes.forEach((type: any) => {
					types.plus((userAgentSpecificTypesToKey as any)[type], item.bytes);
				});

				item.attribution.forEach((url: any) => {
