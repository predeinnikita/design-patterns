...
export function create(options: Partial<KeeperOptions>): PerfKeeper {
	...
	function createEntry(
		...
		if (parent === nil) {
			...
			(warn as any)(`[keeper] Group "${parent.name}" is stopped`);
		} else if (!disabled) {
	...
	function time(this: PerfGroupEntry, name: string, executer?: () => void): PerfEntry | PerfGroupEntry | void {
	...
	function groupEnd(name: string, end?: number): void {
		...
		(warn as any)(`[keeper] Group "${name}" not found`);
	}
